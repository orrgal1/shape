...
# Visual Harness — onboard this repo

Gets the current repo onto the visual-harness canvas: ensure the bridge and web
...
2. **Bridge (WebSocket, port 4400).** Check: `nc -z 127.0.0.1 4400`.
   - **Not running** → start it as a managed background process with the hub tool:
     ```
     hub op:"start" name:"vh-bridge" application:"pnpm"
...
3. **Web (Vite, port 5173, strict).** Check:
   ...
   ```
   hub op:"start" name:"vh-web" application:"pnpm" args:["web"]
       cwd:"$HARNESS" ready:{log: "Local:.*5173"}
   ```

4. **Onboarding survey.** Skip this step if the repo is already mapped —
   `.visual-harness/graph.json` exists in the target with a non-empty `nodes`
   array (`jq -e '.nodes | length > 0' "$target/.visual-harness/graph.json"`);
...
- Do not run the bridge or web in the foreground with bash; they are
  long-running services and MUST be hub-managed (`vh-bridge`, `vh-web`).
